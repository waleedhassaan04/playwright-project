...
test.describe("User API", () => {
  ...
  test("POST register user", async ({ request }) => {
    ...
    const registerResult = await user.registerUser(
      request,
      null,
      BASE_URL,
      userData.REGISTER_USER
    ...
    // EXTRACT THE ACTUAL CREATED USER DETAILS FROM THE SERVER RESPONSE
    credentials.username = registerResult.body.data.username;
    credentials.email = registerResult.body.data.email;
    credentials.password = userData.REGISTER_USER.password;
  });
  ...
  test("GET current user", async ({ request }) => {
    ...
    const getResult = await user.getCurrentUser(
      request,
      token,
      BASE_URL
    );
  ...
  test("PUT replace account", async ({ request }) => {
    ...
    const putResult = await user.replaceAccount(
      request,
      token,
      BASE_URL,
      userData.PUT_USER
    ...
    // SAVE UPDATED USERNAME AND EMAIL
    credentials.username = putResult.newUsername;
    credentials.email = putResult.newEmail;
  ...
  test("PATCH update account", async ({ request }) => {
    ...
    const patchResult = await user.updateAccount(
      request,
      token,
      BASE_URL,
      userData.PATCH_USER
    ...
    // SAVE UPDATED EMAIL
    credentials.email = patchResult.patchEmail;
  ...
  test("POST change password", async ({ request }) => {
    ...
    // SAVE NEW PASSWORD
    credentials.password = changePasswordResult.newPassword;

    console.log("NEW PASSWORD SAVED:", credentials.password);
  });
  ...
  test("DELETE account", async ({ request }) => {
    ...
    const deleteResult = await user.deleteAccount(
      request,
      token,
      BASE_URL,
      credentials
